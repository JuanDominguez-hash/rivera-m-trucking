...
// Prefix used to identify local-auth sessions
export const LOCAL_AUTH_PREFIX = "local_";
...
class SDKServer {
  ...
  private getSessionSecret() {
    const secret = ENV.cookieSecret || "rivera-secret-key-change-in-prod";
    return new TextEncoder().encode(secret);
  }

  async createSessionToken(openId: string, options: { expiresInMs?: number; name?: string } = {}): Promise<string> {
    return this.signSession({ openId, appId: ENV.appId || "rivera-app", name: options.name || "" }, options);
  }
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    // Local auth session (email/password login)
    if (session.openId.startsWith(LOCAL_AUTH_PREFIX)) {
